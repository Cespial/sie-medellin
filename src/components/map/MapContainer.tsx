...
const MAP_VARIABLES: VarConfig[] = [
  ...
  },
  {
    ...
    gradient: "linear-gradient(to right, #F5F5F7, #5856D6, #007AFF)",
    min: "0",
    max: "40K+",
    legendLabels: ["Baja", "Media", "Alta"],
    getColor: (v) => {
...
function buildComunaPopup(
  ...
  const clasifBadges = data.saber11_clasif
    ? Object.entries(data.saber11_clasif)
        ...
        .map(
          ([cl, n]) =>
            `<span style="display:inline-block;padding:1px 6px;border-radius:4px;font-size:10px;font-weight:600;background:${cl === "A+" || cl === "A" ? "rgba(6,214,160,0.2)" : cl === "B" ? "rgba(255,183,3,0.2)" : "rgba(239,35,60,0.2)"};color:${cl === "A+" || cl === "A" ? "#34C759" : cl === "B" ? "#FF9500" : "#FF3B30"};">${cl}: ${n}</span>`
        )
        .join(" ")
    : "";
...
function buildIEPopup(
  ...
  const sectorBadge = `<span style="display:inline-block;padding:2px 8px;border-radius:4px;font-size:10px;font-weight:600;background:${isOficial ? "rgba(0,212,255,0.15)" : "rgba(255,183,3,0.15)"};color:${isOficial ? "#007AFF" : "#FF9500"};">${isOficial ? "Oficial" : "No Oficial"}</span>`;
  const clasifBadge = ieData?.clasificacion
    ? `<span style="display:inline-block;padding:2px 8px;border-radius:4px;font-size:10px;font-weight:600;margin-left:4px;background:${ieData.clasificacion === "A+" || ieData.clasificacion === "A" ? "rgba(6,214,160,0.2)" : ieData.clasificacion === "B" ? "rgba(255,183,3,0.2)" : "rgba(239,35,60,0.2)"};color:${ieData.clasificacion === "A+" || ieData.clasificacion === "A" ? "#34C759" : ieData.clasificacion === "B" ? "#FF9500" : "#FF3B30"};">${ieData.clasificacion}</span>`
    : "";
  ...
  return `<div style="padding:14px;font-family:Inter,system-ui,sans-serif;background:#F5F5F7;color:#E8F4FD;border-radius:12px;min-width:240px;max-width:320px;">
